...
def getSubtitle(key):
      try:
            headers = { 'User-Agent' : 'SubDB/1.0 (Movie Subtitle Downloader/1.0; http://github.com/Godspeed007/Subs-Downloader)' }
...
def main():
      ...
      print ("Hallo frens,desh k vikas hetu subs download krne k liye mai Abhijeet aapka swagat krta hu. \n\n")
      for file in files:
            print ("Opening {0} ...".format(file))
            hashKey=getHash(file)
            exists=check(file)
            if exists:
                  print ("bro subs already downloaded hai for {0}... Skipping \n\n".format(file))
            else:
                  subtitle,status=getSubtitle(hashKey)
                  if not status:
                        print ("bhai iska nhi mil payega File {0}.. Skipping It \n\n".format(file))
                        failed+=1
                  else:
